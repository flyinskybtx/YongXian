...
from _agents import Radar, Commander, Missile, Enemy
...
for r in radars:
    ...
    for commander in commanders:
        if np.linalg.norm(r.src_pos - commander.src_pos) < R2C_DIST:
...
for c in commanders:
    connected_commanders = []
    for commander in commanders:
        if np.linalg.norm(c.src_pos - commander.src_pos) < C2C_DIST:
            connected_commanders.append(commander)
    c.connected_commanders = connected_commanders
    connected_missiles = []
    for missile in missiles:
        if np.linalg.norm(c.src_pos - missile.src_pos) < C2M_DIST:
