...
poses = {
    ...
    "火力": np.random.uniform(-1, 1, (NUM_NODES["火力"], 2)) * DEFEND_AREA_RADIUS,
    "网络攻防": np.random.uniform(-1, 1, (NUM_NODES["网络攻防"], 2)) * DEFEND_AREA_RADIUS,
    ...
    "电子对抗": np.random.uniform(-1, 1, (NUM_NODES["电子对抗"], 2)) * DEFEND_AREA_RADIUS,
}
        ...
        if key1 + '-' + key2 in RULES:  # 符合连接规则
            ...
            for n1 in nodes1:
                ...
                connectable_n2 = [n2 for n2 in nodes2
                                  if np.linalg.norm(n1.src_pos - n2.src_pos) <= CONNECTION_RADIUS and
